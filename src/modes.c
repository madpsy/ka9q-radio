...
int loadpreset(struct channel *chan,dictionary const *table,char const *sname){
  ...
  fprintf(stderr,"loadpreset: sname='%s', demod_name='%s'\n",sname,demod_name ? demod_name : "(null)");
  if(demod_name){
    int const x = demod_type_from_name(demod_name);
    fprintf(stderr,"loadpreset: demod_type_from_name('%s') returned %d\n",demod_name,x);
    if(x >= 0)
      chan->demod_type = x;
  }
  {
  ...
  chan->spectrum.bin_count = config_getint(table,sname,"bin-count",chan->spectrum.bin_count);
  if(chan->spectrum.bin_count < 0)
    chan->spectrum.bin_count = 0;
  
  chan->spectrum.bin_bw = config_getfloat(table,sname,"bin-bw",chan->spectrum.bin_bw);
  if(chan->spectrum.bin_bw < 0)
    chan->spectrum.bin_bw = 0;
