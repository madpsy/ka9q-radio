...
int send_radio_status(struct sockaddr const *sock,struct frontend const *frontend,struct channel *chan){
  return send_radio_status_ex(sock,frontend,chan,false);
}

// Extended version that can skip spectrum_poll
static int send_radio_status_ex(struct sockaddr const *sock,struct frontend const *frontend,struct channel *chan,bool skip_spectrum_poll){
...
static bool decode_radio_commands_with_source(struct channel *chan,uint8_t const *buffer,int length,char const *source_ip){
  ...
  // These are set to sentinel values indicating "not specified"
  float override_low_edge = NAN;
  float override_high_edge = NAN;
  int override_bin_count = -1;
  float override_bin_bw = NAN;
  bool has_overrides = false;
  bool spectrum_params_changed = false; // Track if spectrum parameters changed
 ...
 if(isfinite(f)){
   // Save for potential override after PRESET
   override_low_edge = f;
   has_overrides = true;
   // Only apply immediately for non-spectrum channels
   // Spectrum channels will be handled by the override section after PRESET
   if(chan->demod_type != SPECT_DEMOD){
     chan->filter.min_IF = max(f,-(float)chan->output.samprate/2);
     new_filter_needed = true;
     if(Verbose > 1)
       fprintf(stderr,"set ssrc %u LOW_EDGE = %.1f Hz\n",ssrc,f);
   }
 }
      }
      break;
 ...
 if(isfinite(f)){
   ...
   if(chan->demod_type != SPECT_DEMOD){
     ...
     new_filter_needed = true;
     if(Verbose > 1)
       fprintf(stderr,"set ssrc %u HIGH_EDGE = %.1f Hz\n",ssrc,f);
   }
 }
      }
      break;
 ...
 if(isfinite(x)){
   ...
   override_bin_bw = x;
   has_overrides = true;
   // Don't apply immediately - will be applied after PRESET processing
 }
      }
      break;
    case BIN_COUNT:
 ...
 setport(&chan->status.dest_socket,DEFAULT_STAT_PORT);
  ...
  // Apply parameter overrides after PRESET processing
  // This allows dynamic channel creation to override preset defaults
  if(has_overrides && chan->demod_type == SPECT_DEMOD){
    if(Verbose > 1)
      fprintf(stderr,"Applying parameter overrides after preset for ssrc %u\n",ssrc);
    
    // For spectrum channels, LOW_EDGE and HIGH_EDGE are informational only
    // They don't actually control filtering - spectrum analysis uses the full bandwidth
    // Just log them but don't try to set filter fields (spectrum channels don't use chan->filter)
    if(isfinite(override_low_edge)){
      if(Verbose > 1)
 fprintf(stderr,"  Override LOW_EDGE: %.1f Hz (informational for spectrum)\n",override_low_edge);
    }
    
    if(isfinite(override_high_edge)){
      if(Verbose > 1)
 fprintf(stderr,"  Override HIGH_EDGE: %.1f Hz (informational for spectrum)\n",override_high_edge);
    }
    
    // For spectrum channels, changing bin_count or bin_bw requires reallocation
    // We CANNOT free bin_data here because the spectrum thread may be using it
    // We just update the parameters and let the spectrum thread handle everything
    // spectrum_poll() will be skipped because bin_data will be NULL after the
    // spectrum thread frees and before it reallocates
    
    if(override_bin_count > 0 && override_bin_count != chan->spectrum.bin_count){
      if(Verbose > 1)
        fprintf(stderr,"  Override BIN_COUNT: %d -> %d (spectrum thread will reallocate)\n",
                chan->spectrum.bin_count, override_bin_count);
      chan->spectrum.bin_count = override_bin_count;
      spectrum_params_changed = true;
    }
    
    if(isfinite(override_bin_bw) && override_bin_bw != chan->spectrum.bin_bw){
      if(Verbose > 1)
        fprintf(stderr,"  Override BIN_BW: %.1f -> %.1f Hz (spectrum thread will reallocate)\n",
                chan->spectrum.bin_bw, override_bin_bw);
      chan->spectrum.bin_bw = override_bin_bw;
      spectrum_params_changed = true;
    }
    
    // Note: We don't touch bin_data here - the spectrum thread owns it
    // We'll pass a flag to send_radio_status to skip spectrum_poll
    if(spectrum_params_changed && Verbose > 1){
      fprintf(stderr,"Spectrum params changed for ssrc %u, will skip spectrum_poll until reallocation\n", ssrc);
    }
  }
  
  if(chan->demod_type == SPECT_DEMOD){
    memset(chan->preset,0,sizeof(chan->preset)); // No presets in this mode
    // For spectrum channels, return whether parameters changed (not restart_needed)
    if(spectrum_params_changed){
      if(Verbose > 1)
        fprintf(stderr,"spectrum params changed for ssrc %u\n",ssrc);
      return true; // Signal to skip spectrum_poll
    }
  }
  ...
  case SPECT_DEMOD:
    {
      ...
      // Don't call spectrum_poll() if explicitly skipped or if bin_data is NULL
      // This can happen right after parameter changes, before the spectrum thread reallocates
      if(!skip_spectrum_poll && chan->spectrum.bin_data != NULL){
        spectrum_poll(chan); // Update the spectral data (wide bins only)
      } else if(Verbose > 1){
        fprintf(stderr,"Skipping spectrum_poll for ssrc %u (skip_flag=%d, bin_data=%p)\n",
                chan->output.rtp.ssrc, skip_spectrum_poll, (void*)chan->spectrum.bin_data);
      }
