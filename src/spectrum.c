...
int demod_spectrum(void *arg){
  ...
  while(1){
	...
	plan = NULL;
      ...
      chan->spectrum.allocated_bin_count = bin_count; // Record actual allocation size
	...
	if(r != 0){
	  fprintf(stderr,"ERROR: create_filter_output failed for spectrum channel ssrc 0x%08x (samprate=%d, frame_len=%d)\n",
		  chan->output.rtp.ssrc, samprate, frame_len);
	  fprintf(stderr,"  Requested parameters are invalid. Try increasing bin_bw (currently %.1f Hz)\n", bin_bw);
...
int spectrum_poll(struct channel *chan){
  ...
  // Safety check: verify that bin_data allocation matches requested bin_count
  // This prevents crashes when parameters change but reallocation hasn't happened yet
  int const bin_count = chan->spectrum.bin_count <= 0 ? 64 : chan->spectrum.bin_count;
  if(chan->spectrum.allocated_bin_count != bin_count){
    // Parameters have changed but spectrum thread hasn't reallocated yet
    // Skip this poll to avoid buffer overflow
    if(Verbose > 1)
      fprintf(stderr,"spectrum_poll skipped for ssrc %u: allocated_bin_count=%d != requested bin_count=%d (waiting for reallocation)\n",
              chan->output.rtp.ssrc, chan->spectrum.allocated_bin_count, bin_count);
    return -1;
  }
  ...
  int const M = frontend->M;
  int const N = L + M - 1;
  // bin_count already declared above at line 264
  ...
  if(master->in_type == COMPLEX){
    ...
    // Special handling for full-bandwidth real input (e.g., RX888 0-30 MHz)
    if(requested_start <= 0 && requested_end >= master->bins - 1){
      // Covering full or nearly full bandwidth - map linearly from available bins
      // This fixes the bug where we'd get zeros at the edges
      for(int i = 0; i < input_bins; i++){
        // Map output bin i to input bin range
      ...
      // Original logic for partial bandwidth coverage
      int binp = requested_start;
      int i = 0;
    ...
    // Special handling for full-bandwidth inverted real input
    if(requested_start <= 0 && requested_end >= master->bins - 1){
      // Covering full bandwidth - map linearly in reverse
      for(int i = 0; i < input_bins; i++){
        double bin_ratio = (double)master->bins / input_bins;
        int binp = master->bins - 1 - (int)(i * bin_ratio);
        if(binp >= 0 && binp < master->bins)
          power_buffer[i] = cnrmf(fdomain[binp]);
        else
          power_buffer[i] = 0;
  ...
  // Merge the bins
  // For real inputs covering full bandwidth, use linear mapping
  // For complex inputs or partial coverage, use the original split logic
  
  // Detect if this is a real input covering full bandwidth
  ...
  if(is_real_full_bw){
    // Real input covering DC to Nyquist - use simple linear mapping
    // All frequencies are positive, no negative frequency bins
    double ratio = (double)input_bins / bin_count;
    
    for(int out = 0; out < bin_count; out++){
      double in_start = out * ratio;
      double in_end = (out + 1) * ratio;
      int in_start_int = (int)in_start;
      int in_end_int = (int)ceil(in_end);
      
      double p = 0;
      int count = 0;
      for(int in = in_start_int; in < in_end_int && in < input_bins; in++){
        assert(in >= 0 && in < input_bins);
        p += power_buffer[in];
        count++;
      }
      // Average the bins that contribute to this output bin
      if(count > 0)
        chan->spectrum.bin_data[out] = (p * gain);
      else
        chan->spectrum.bin_data[out] = 0;
    }
  } else {
    // Original logic for complex inputs or partial real input coverage
