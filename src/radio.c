...
int loadconfig(char const *file){
  ...
  Affinity = config_getboolean(Configtable,GLOBAL,"affinity",false);
  {
    ...
    // Debug: print all loaded presets
    int const npresets = iniparser_getnsec(Preset_table);
    fprintf(stderr,"Loaded %d presets:",npresets);
    for(int i = 0; i < npresets; i++){
      char const *preset_name = iniparser_getsecname(Preset_table,i);
      fprintf(stderr," %s",preset_name);
    }
    fprintf(stderr,"\n");
  }
